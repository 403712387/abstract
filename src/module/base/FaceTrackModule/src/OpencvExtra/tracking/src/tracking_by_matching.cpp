...
float TrackerByMatching::ShapeAffinity(float weight, const cv::Rect &trk,
                                       const cv::Rect &det) {
    float w_dist = static_cast<float>(std::fabs((float)(trk.width - det.width)) / (float)(trk.width + det.width));
    float h_dist = static_cast<float>(std::fabs((float)(trk.height - det.height)) / (float)(trk.height + det.height));
