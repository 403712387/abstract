              ...
              const char *upload_data, size_t *upload_data_size, void **con_cls)
{
    ...
    else if (requestUrl.compare("/start/abstract", Qt::CaseInsensitive) == 0)  // 开始提取人脸/人体信息
    {
        response = onStartAbstract(body);
    }
    else if (requestUrl.compare("/stop/abstract", Qt::CaseInsensitive) == 0)  // 停止提取人脸/人体信息
    {
        response = onStopAbstract(body);
    }
...
// 下发提取人脸/人体信息的任务
std::string HttpModule::onStartAbstract(std::string &body)
{
    std::pair<std::shared_ptr<AbstractCondition>, std::shared_ptr<Error>> ret = JsonHelper::parseAbstractCondition(body);
    if (NULL != ret.second)
    {
        return getResponseBody(400, ret.second->getErrorReason());
    }

    // 发送消息
    std::shared_ptr<AbstractStartMessage> message = std::make_shared<AbstractStartMessage>(ret.first, Sync_Trans_Message);
    std::shared_ptr<BaseResponse> response = sendMessage(message);

    // 处理回应
    std::shared_ptr<AbstractStartResponse> abstractResponse = std::dynamic_pointer_cast<AbstractStartResponse>(response);
    if (NULL == abstractResponse.get())
    {
        return getResponseBody(500, "not process request");
    }
    else if(!abstractResponse->isSuccessfulProcess())
    {
        return getResponseBody(501, abstractResponse->getErrorReason());
    }

    return JsonHelper::abstraceIdToJson(abstractResponse->getAbstractId()).toStyledString();
}

// 停止提取人脸/人体信息
std::string HttpModule::onStopAbstract(std::string &body)
{
    std::pair<std::shared_ptr<StopAbstract>, std::shared_ptr<Error>> ret = JsonHelper::parseStopAbstract(body);
    if (NULL != ret.second.get())
    {
        LOG_E(mClassName, "parse abstract id fail, reason:" << ret.second->getErrorReason() << ", body:" << body);
        return getResponseBody(400, ret.second->getErrorReason());
    }

    // 发送消息
    std::shared_ptr<AbstractStopMessage> message = std::make_shared<AbstractStopMessage>(ret.first);
    sendMessage(message);
    return getResponseBody(200, "OK");
}
...
{
    std::string result = "Welcome to abstract service";
    return result;
}
