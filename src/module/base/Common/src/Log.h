...
namespace common_log
{
    ...
    ABSTRACT_EXPORT void setLogConfigFile(const std::string fileConfig);

	// 设置日志级别
    ABSTRACT_EXPORT void setLogLevel(Log_Level logLevel);

    // 获取日志级别
    ABSTRACT_EXPORT Log_Level getLogLevel();

	// 设置过滤关键
    ABSTRACT_EXPORT void setFilterWord(const std::string keyWord);

    // 写日志(forceWrite如果为true,无论日志级别，强制写入)
    ABSTRACT_EXPORT void writeLog(const std::string &info, Log_Level logLevel = Log_Level_Info, bool forceWrite = false);
}
